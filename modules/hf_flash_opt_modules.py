...
class OPTAttention(_OPTAttention):
    def __init__(
        ...
        if flash_attn_installed:
            self.flash_attn = FlashAttention(softmax_scale=self.scaling, attention_dropout = dropout)
...
class GPTBlock(OPTDecoderLayer):
    def __init__(self, config, *args, use_checkpoint=True, device='cpu', **kargs):
        ...
        if apex_installed:
            self.self_attn_layer_norm = apex.normalization.FusedLayerNorm(self.embed_dim)
