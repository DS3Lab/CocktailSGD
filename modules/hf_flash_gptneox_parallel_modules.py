...
from transformers.models.gpt_neox.modeling_gpt_neox import ACT2FN
# from transformers.models.gpt_neox.modeling_gpt_neox import RotaryEmbedding

from comm.comm_utils import (
    get_pipeline_parallel_comm, get_pipeline_parallel_rank, get_pipeline_parallel_world_size,
    mp_all_reduce, mp_all_gather
)
...
class GPTNeoXAttention(_GPTNeoXAttention):
    ...
    def __init__(self, config):
        ...
        self.num_attention_heads = config.num_attention_heads // get_pipeline_parallel_world_size()
        self.hidden_size = config.hidden_size
        self.head_size = self.hidden_size // self.num_attention_heads // get_pipeline_parallel_world_size()
        ...
        self.query_key_value = nn.Linear(config.hidden_size, 3 * self.num_attention_heads * self.head_size)
        self.dense = nn.Linear(self.num_attention_heads * self.head_size, config.hidden_size)
        
        assert flash_attn_installed
        self.flash_attn = FlashAttention(softmax_scale=1.0/self.norm_factor, attention_dropout = 0)
        
    def load_from_full_layer(self):
        pass
    ...
    def forward(
        ...
        # comm
        attn_output = mp_all_reduce(attn_output)
...
class GPTNeoXMLP(_GPTNeoXMLP):
    def __init__(self, config):
        super(_GPTNeoXMLP, self).__init__()
        self.dense_h_to_4h = nn.Linear(config.hidden_size, config.intermediate_size // get_pipeline_parallel_world_size())
        self.dense_4h_to_h = nn.Linear(config.intermediate_size // get_pipeline_parallel_world_size(), config.hidden_size)
...
class GPTEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.embed_dim = config.hidden_size // get_pipeline_parallel_world_size()
    ...
    def forward(self, input_ids, *args, **kargs):
        ...
        hidden_states = mp_all_gather(hidden_states)
...
class GPTLMHead(nn.Module):
    def __init__(self, config):
        ...
        self.hidden_size_per_gpu = config.hidden_size // get_pipeline_parallel_world_size()
        self.embed_out = nn.Linear(self.hidden_size_per_gpu, config.vocab_size, bias=False)
    ...
    def forward(self, x, *args, **kargs):
        ...
        i = get_pipeline_parallel_rank()
        x = x[:, :, self.hidden_size_per_gpu*i: self.hidden_size_per_gpu*(1+i)]
