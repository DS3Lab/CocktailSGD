...
export WANDB_NAME=RP-7B-1TT-test-tp
...
--model-type flash_gptneox_parallel \
--optimizer fusedadam \
--seed 42 \
--task-name rp_arxiv_small \
--checkpoint-path ./model_ckpts/$WANDB_NAME \
--num-layers 32 --embedding-dim 4096 \
--total-steps 200000 --warmup-steps 10 --train-warmup-steps 0 \
--stop-steps 8001 \
...
--dp-backend nccl \
--dp-mode allreduce \
--pp-mode nogpipe --profiling no-profiling"

(trap 'kill 0' SIGINT; \
python dist_lm_train_tp.py $(echo ${ARGS}) --cuda-id 0 --rank 0 \
    & \
python dist_lm_train_tp.py $(echo ${ARGS}) --cuda-id 1 --rank 1 \
    & \
python dist_lm_train_tp.py $(echo ${ARGS}) --cuda-id 2 --rank 2 \
    & \
python dist_lm_train_tp.py $(echo ${ARGS}) --cuda-id 3 --rank 3 \
    & \
python dist_lm_train_tp.py $(echo ${ARGS}) --cuda-id 4 --rank 4 \
    & \
python dist_lm_train_tp.py $(echo ${ARGS}) --cuda-id 5 --rank 5 \
    & \
python dist_lm_train_tp.py $(echo ${ARGS}) --cuda-id 6 --rank 6 \
    & \
python dist_lm_train_tp.py $(echo ${ARGS}) --cuda-id 7 --rank 7 \
    & \
wait)
